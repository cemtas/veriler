...
[data-testid="stAppViewContainer"] { background-color: #050810; overflow: hidden; overflow-y: auto;} 
...
.scroll-indicator { text-align: center; margin-top: 60px; margin-bottom: 60px; animation: bounce 2s infinite; color: rgba(0, 229, 255, 0.7); font-family: 'Orbitron', sans-serif; font-size: 18px; letter-spacing: 2px; }
...
def canli_ve_altbant_verilerini_cek():
    ...
    def get_data_safe(ticker, inv_url, is_tr):
        ...
        # 3. Yfinance Fallback (Fastinfo hatası giderildi, history kullanıldı)
        if val == 0.0 and ticker:
            try:
                ...
                if not hist.empty:
                    ...
                    hist2 = yf.Ticker(ticker).history(period="2d")
                    if not hist2.empty:
                        val = float(hist2['Close'].iloc[-1])
            except: pass
    ...
    gram_g, _ = get_data_safe("GC=F", "https://tr.investing.com/currencies/gau-try", True)
    if gram_g > 0: 
...
with top_empty.container():
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; padding: 10px 15px; margin-bottom: 10px;">
        <div style="font-family: 'Orbitron', sans-serif; font-size: 38px; color: #4ade80; font-weight: 700; text-shadow: 0px 0px 10px rgba(74,222,128,0.4);">{get_tarih_formatsiz()}</div>
...
# AŞAĞI KAYDIRMA UYARISI
st.markdown("<div class='scroll-indicator'>▼ SCROLL DOWN FOR AVERAGE (M-1) ▼</div>", unsafe_allow_html=True)
...
<div style="text-align: center; margin-top: 15px; margin-bottom: 60px; position: relative; z-index: 10;">
    <span style="font-family: 'Segoe UI Light', 'Helvetica Neue', Arial, sans-serif; font-size: 22px; font-weight: 500; color: #cbd5e1; letter-spacing: 3px; text-shadow: 0px 1px 3px rgba(0,0,0,0.8);">Powered by Cem TAŞ</span>
</div>
"""
